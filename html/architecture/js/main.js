...
function mainSliderNavPosition() {
	jQuery('.page_mainslider').find('.slide_description h2').each(function() {
		var $slideHeading = jQuery(this);
		var $sliderNav = jQuery('.page_mainslider').find('.flex-control-nav');
		var headingFontsize = parseInt($slideHeading.css('font-size'));
		$slideHeading.css('margin-bottom', headingFontsize * 1.2);
		var headingOffset = $slideHeading.offset().top;
		var navTopPosition = headingOffset - jQuery('.page_mainslider').offset().top + $slideHeading.height() + headingFontsize * 0.52;
		$sliderNav.css('top', navTopPosition);
	});
}
